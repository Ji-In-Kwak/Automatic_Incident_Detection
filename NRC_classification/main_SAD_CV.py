...
###############################################################
    ...
    os.makedirs(f'./checkpoints_SAD_CV/{args.dataset}', exist_ok=True)
    os.makedirs(f'./log_SAD_CV', exist_ok=True)
    checkpoints_path=f'./checkpoints_SAD_CV/{args.dataset}/{args.exp_name}+bestcheckpoint.pt'
    logging.basicConfig(filename=f"./log_SAD_CV/{args.exp_name}.log",filemode="a",format="%(asctime)s-%(name)s-%(levelname)s-%(message)s",level=logging.INFO)
    logger=logging.getLogger('DeepSAD')


    # DataLoader
#     train_loader, _, test_loader = traffic_mtsc_loader(args)
...
#     model = DeepSAD_trainer.train(args, logger, train_loader, test_loader, test_loader, model, checkpoints_path)
