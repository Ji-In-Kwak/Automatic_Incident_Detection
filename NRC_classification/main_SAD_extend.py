...
from optim import DeepSAD_ext_trainer
...
###############################################################
    ...
    os.makedirs(f'./checkpoints_SAD_extend/{args.dataset}', exist_ok=True)
    os.makedirs(f'./log_SAD_extend', exist_ok=True)
    checkpoints_path=f'./checkpoints_SAD_extend/{args.dataset}/{args.exp_name}+bestcheckpoint.pt'
    logging.basicConfig(filename=f"./log_SAD_extend/{args.exp_name}.log",filemode="a",format="%(asctime)s-%(name)s-%(levelname)s-%(message)s",level=logging.INFO)
    ...
    model = DeepSAD_ext_trainer.train(args, logger, train_loader, val_loader, test_loader, model, checkpoints_path)
